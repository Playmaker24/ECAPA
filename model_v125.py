...
class NormalizationLayer(nn.Module):
    ...
    def forward(self, x):
        batch_size, channels, frames, n_mels = x.size()
        x = x.view(batch_size, -1).cpu().numpy()
        scaler = StandardScaler()
        x = scaler.fit_transform(x)
        x = torch.tensor(x).view(batch_size, channels, frames, n_mels).cuda()
        return x
"""
class ECAPA_TDNN(nn.Module):
    ...
    def __init__(self, C):
        ...
        #self.normalizer = NormalizationLayer() # Normalize feature
